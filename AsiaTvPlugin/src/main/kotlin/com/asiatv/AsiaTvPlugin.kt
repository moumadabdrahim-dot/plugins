...
class AsiaTvPlugin : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val items = doc.select("article.post, .post-item, .MovieBlock").mapNotNull {
            val a = it.selectFirst("a") ?: return@mapNotNull null
            val href = a.attr("href")
            val title = it.selectFirst("h2, h3, .Title, .BlockTitle")?.text()?.trim()
                ?: a.attr("title").trim()
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("article.post, .post-item, .MovieBlock").mapNotNull {
            val a = it.selectFirst("a") ?: return@mapNotNull null
            val href = a.attr("href")
            val title = it.selectFirst("h2, h3, .Title, .BlockTitle")?.text()?.trim()
                ?: a.attr("title").trim()
            val img = it.selectFirst("img")?.attr("src")
                ?: it.selectFirst("img")?.attr("data-src") ?: ""
            val isMovie = href.contains("film") || title.contains("فيلم")
            if (isMovie) {
                newMovieSearchResponse(title, href, TvType.Movie) { this.posterUrl = img }
            } else {
                newTvSeriesSearchResponse(title, href, TvType.AsianDrama) { this.posterUrl = img }
            }
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val title = doc.selectFirst("h1.entry-title, h1, .Title")?.text()?.trim() ?: return null
        val poster = doc.selectFirst(".post-thumbnail img, .entry-content img, article img")
            ?.let { it.attr("src").ifBlank { it.attr("data-src") } } ?: ""
        val description = doc.selectFirst(".entry-content p, p.anime-story, .Description")?.text()?.trim() ?: ""
        val genres = doc.select(".genre a, ul.anime-genres li a, a[rel='category tag']").map { it.text() }
        val isMovie = url.contains("film") || title.contains("فيلم")

        // قائمة الحلقات
        val episodes = doc.select(".seasons-episodes a, ul.episodesList li a, .EpisodesList a").mapNotNull { ep ->
            val epUrl = ep.attr("href")
            val epName = ep.text().trim()
            val epNum = Regex("\\d+").find(epName)?.value?.toIntOrNull() ?: 1
            Episode(epUrl, epName, episode = epNum)
        }
    ...
    override suspend fun loadLinks(
        ...
        doc.select("[data-link], [data-src], [data-url]").forEach {
            val link = it.attr("data-link").ifBlank { it.attr("data-src") }.ifBlank { it.attr("data-url") }
            if (link.isNotBlank()) loadExtractor(link, data, subtitleCallback, callback)
        }

        // 3. فك تشفير Packer (eval) — الطريقة الأساسية
        val packerRegex = Regex("""eval\(function\(p,a,c,k,e,d\)\{.*?\}\('(.+?)',(\d+),\d+,'(.+?)'""", RegexOption.DOT_MATCHES_ALL)
        packerRegex.find(html)?.let { match ->
            val unpacked = unpackPacker(match.groupValues[1], match.groupValues[2].toInt(), match.groupValues[3].split("|"))
        ...
        val directM3u8 = Regex("""https?://[^\s"'<>]+\.m3u8[^\s"'<>]*""").findAll(html)
        directM3u8.forEach {
            callback(
                ExtractorLink(
                    source = name,
    ...
    // فك تشفير Packer
    // ==============================
    private fun unpackPacker(payload: String, radix: Int, keys: List<String>): String {
        var result = payload
        keys.forEachIndexed { index, key ->
            if (key.isNotEmpty()) {
                val base36 = index.toString(radix)
                result = result.replace(Regex("\\b$base36\\b"), key)
            }
        }
        return result
    }
